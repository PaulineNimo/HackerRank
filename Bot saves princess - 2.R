...
data <- read.table("stdin",sep = "",comment.char = "",stringsAsFactors = F,fill = TRUE,header=F,row.names = NULL)
...
nextMove <- function(N,r,c,grid){
  ...
  if(no_row < 100){
    ...
    if(no_row > 0){
      for(i in 1:(no_row)){
        row_p <- unlist(strsplit(grid[i,1],split = ""))
        check_p <- which(row_p %in% "p")
        if(length(check_p) > 0){
          which_col_p <- check_p
          which_row_p <- i - 1
        }
        row_m <- unlist(strsplit(grid[i,1],split = ""))
    ...
    if(length(moves) > 0){
      write.table(cat(format(moves_real[1], nsmall=1), sep="\n"), sep = "", append=T, row.names = F, col.names = F)
    }
  }
